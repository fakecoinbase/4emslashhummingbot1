...
cdef class PureMarketMakingStrategyV2(StrategyBase):
    ...
    def __init__(self,
        ...
        self._order_mode_buy = True
        self._order_mode_sell = False
        self._order_first_buy = True
        self._order_first_sell = False
        self._price_order = 0
        ...
        if market_info is not None:
            ...
            if order_filled_event.trade_type is TradeType.BUY:
                if self._logging_options & self.OPTION_LOG_MAKER_ORDER_FILLED:
                    self._order_mod_buy = False
                    self._order_mod_sell = True
                    if self._first_order_sell:
                        self._first_order_buy = False
                        self._first_order_sell = False
                    self.log_with_clock(
                        logging.INFO,
                ...
                if self._logging_options & self.OPTION_LOG_MAKER_ORDER_FILLED:
                    self._order_mod_buy = True
                    self._order_mod_sell = False
                    if self._first_order_buy:
                        self._first_order_buy = False
                        self._first_order_sell = False
                    self.log_with_clock(
                        logging.INFO,
    ...
    cdef c_execute_orders_proposal(self, object market_info, object orders_proposal):
        ...
        if actions & ORDER_PROPOSAL_ACTION_CREATE_ORDERS:
            if len(orders_proposal.buy_order_sizes) > 0 and orders_proposal.buy_order_sizes[0] > 0 and self._order_mod_buy==True:
                if self._first_order_buy:
                    if orders_proposal.buy_order_type is OrderType.LIMIT and orders_proposal.buy_order_prices[0] > 0:
                        ...
                        for idx in range(len(orders_proposal.buy_order_sizes)):
                            ...
                            self._order_price = orders_proposal.buy_order_prices[idx]
                    ...
                    if orders_proposal.buy_order_type is OrderType.LIMIT and orders_proposal.buy_order_prices[0] > 0:
                        ...
                        for idx in range(len(orders_proposal.buy_order_sizes)):
                            if orders_proposal.buy_order_prices[idx] > self._order_price
                                orders_proposal.buy_order_prices[idx] = self._order_price
                            bid_order_id = self.c_buy_with_specific_market(
                                market_info,
                                orders_proposal.buy_order_sizes[idx],
                                order_type=OrderType.LIMIT,
            ...
            if len(orders_proposal.sell_order_sizes) > 0 and orders_proposal.sell_order_sizes[0] > 0 and self._order_mod_sell==True:
                if self._first_order_sell:
                    if orders_proposal.sell_order_type is OrderType.LIMIT and orders_proposal.sell_order_prices[0] > 0:
                        ...
                        for idx in range(len(orders_proposal.sell_order_sizes)):
                            ...
                            self._order_price = orders_proposal.sell_order_prices[idx]
                    ...
                    if orders_proposal.sell_order_type is OrderType.LIMIT and orders_proposal.sell_order_prices[0] > 0:
                        ...
                        for idx in range(len(orders_proposal.sell_order_sizes)):
                            if orders_proposal.sell_order_prices[idx] < self._order_price
                                orders_proposal.sell_order_prices[idx] = self._order_price
                            ask_order_id = self.c_sell_with_specific_market(
                                market_info,
                                orders_proposal.sell_order_sizes[idx],
                                order_type=OrderType.LIMIT,
